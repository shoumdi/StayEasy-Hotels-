...
<main class="size-full grid place-items-center">
    <form action="{{route('admin.roles.update')}}" method="post" class="w-[35%]">
        ...
        <div class="flex flex-col">
            ...
            <textarea type="description" name="description" id="description" placeholder="write something"
                class="w-full resize-none bg-white/[0.03] border border-neutral-500/10 rounded-xl px-4 py-3 text-sm font-bold text-neutral-800 placeholder:text-slate-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-all duration-200">
            {{$role->description ?? ''}}
            </textarea>
        </div>
